//In this we will learn about std::mutex::try_lock() On Mutex In C++11 Threading.
...
//	t1.join();
//	t2.join();
//	std::cout << counter << std::endl; // it always give us the anwer 2 lakh as std::mutex::lock() is a blocking state if any thread is not able acquire the lock then it wait until he cant get lock
//	return 0;
//}
...
// using try_lock() it gives us the random value if thread able to acquire the lock then it is able to increment the counter otherwiswe it will return skip which result in skipping of its chance
